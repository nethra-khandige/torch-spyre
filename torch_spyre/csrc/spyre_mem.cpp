...
const at::Tensor& spyre_resize_(
  ...
  //  (b) the SpyreTensorLayout compared by the recompile guard
  //      (_spyre_TENSOR_MATCH in _monkey_patch.py) stays identical across
  //      every concrete shape the reservation covers, so resizing never
  //      forces a spurious recompile.
  std::vector<int64_t> layout_size = size_int.vec();
  if (self_impl->reserved_dim.has_value()) {
    const int64_t rdim = *self_impl->reserved_dim;
    TORCH_CHECK(
        rdim < static_cast<int64_t>(size_int.size()),
        "resize_ dropped the reserved dimension of a max-reserved tensor");
